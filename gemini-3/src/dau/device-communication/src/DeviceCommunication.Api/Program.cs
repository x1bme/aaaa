...
using YourNamespace.Data; // Update with your actual namespace for GeminiDbContext
...
builder.Services.AddGrpcClient<ArchiverService.ArchiverServiceClient>(o =>
{
    var address = builder.Configuration["ArchiverService:Address"]; // Use address in json file
    if (string.IsNullOrEmpty(address))
    {
        throw new InvalidOperationException("ArchiverService:Address is not configured in appsettings.json");
    }
...
builder.Services.AddSingleton<DataForwardingService>();
...
app.Services.GetRequiredService<DataForwardingService>();
