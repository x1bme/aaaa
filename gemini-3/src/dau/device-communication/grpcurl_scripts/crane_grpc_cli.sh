...
# Usage: grpcFullUpdateStm32
# Uses JSON file for input parameters
grpcFullUpdateStm32() {
    _call_grpc "UpdateFirmware" "@${PAYLOAD_DIR}/update_firmware_stm32.json"
}

# Usage: grpcFullUpdateEsp32
# Uses JSON file for input parameters
grpcFullUpdateEsp32() {
    _call_grpc "UpdateFirmware" "@${PAYLOAD_DIR}/update_firmware_esp32.json"
}
...
_ensure_payload_templates() {
    ...
    local files_to_create_if_missing=(
        ...
        "firmware_prepare_request.json:{\"device_id\": \"${DEFAULT_DEVICE_ID}\",\"firmware_size_bytes\": 10240,\"block_size_preference\":512, \"type\": \"FIRM_IMG_TYPE_GRPC_STM32\"}"
        "firmware_verify_request.json:{\"device_id\": \"${DEFAULT_DEVICE_ID}\",\"operation\": \"FW_OP_GRPC_VERIFY\", \"type\": \"FIRM_IMG_TYPE_GRPC_STM32\"}"
        "firmware_apply_request.json:{\"device_id\": \"${DEFAULT_DEVICE_ID}\",\"operation\": \"FW_OP_GRPC_APPLY\",\"reboot_delay_seconds_for_apply\":5, \"type\": \"FIRM_IMG_TYPE_GRPC_STM32\"}"
        "firmware_abort_request.json:{\"device_id\": \"${DEFAULT_DEVICE_ID}\",\"operation\": \"FW_OP_GRPC_ABORT\",\"reason_for_abort\":\"Test Abort from CLI\", \"type\": \"FIRM_IMG_TYPE_GRPC_STM32\"}"
        "update_firmware_stm32.json:{\"device_id\":\"${DEFAULT_DEVICE_ID}\",\"version\":\"4.0.0\",\"firmware_data\":\"aGh1YSB3YXMgaGVyZQo=\",\"type\":\"FIRM_IMG_TYPE_GRPC_STM32\"}"
        "update_firmware_esp32.json:{\"device_id\":\"${DEFAULT_DEVICE_ID}\",\"version\":\"4.0.1\",\"firmware_data\":\"aGh1YSB3YXMgaGVyZQo=\",\"type\":\"FIRM_IMG_TYPE_GRPC_ESP32\"}"
...
grpcHelp() {
    ...
    echo
    echo "  -- Step-by-step Firmware Update --"
    ...
    echo
    echo "  -- Full Firmware Update (All-in-one) --"
    echo "  grpcFullUpdateStm32          - Uses '${PAYLOAD_DIR}/update_firmware_stm32.json'"
    echo "  grpcFullUpdateEsp32          - Uses '${PAYLOAD_DIR}/update_firmware_esp32.json'"
    echo
    echo "  -- Data & Capture --"
