...
namespace DeviceCommunication.Infrastructure.Services
{
    public class DeviceCommandOrchestrator : IDisposable
    {
        ...
        public Task<Device.UpdateFirmwareResponse> SendPrepareFirmwareUpdateAndAwaitResponseAsync(string deviceId, string serverId, uint firmwareSizeBytes, string firmwareVersion, ByteString signature, uint blockSizePreference, TimeSpan timeout)
        {
            var currentSequence = (uint)Interlocked.Increment(ref _orchestratorSequenceCounter);
            var requestMain = new Device.Main {
                Header = new Device.Header { DeviceId = serverId, SequenceNumber = currentSequence, TimestampMs = (ulong)DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() },
                FirmwareRequest = new FirmwareRequest { CommandType = FirmwareCommandType.Update, Update = new UpdateFirmwareRequest { Operation = FirmwareUpdateOperation.FirmwareOpPrepare, Prepare = new FirmwarePreparePayload { FirmwareSizeBytes = firmwareSizeBytes, FirmwareVersion = firmwareVersion, Signature = signature, BlockSizePreference = blockSizePreference } } }
            };
            return SendCommandAndAwaitSpecificResponseAsync(deviceId, serverId, requestMain, timeout,
                response => (response.PayloadCase == Device.Main.PayloadOneofCase.FirmwareResponse && response.FirmwareResponse.CommandResponsePayloadCase == Device.FirmwareResponse.CommandResponsePayloadOneofCase.Update && response.FirmwareResponse.Update.Operation == FirmwareUpdateOperation.FirmwareOpPrepare)
                            ? response.FirmwareResponse.Update : null, // Returns the whole UpdateFirmwareResponse
                "PrepareFirmwareUpdate");
        }

        public Task<Device.UpdateFirmwareResponse> SendExecuteFirmwareUpdateOperationAndAwaitResponseAsync(string deviceId, string serverId, Device.UpdateFirmwareRequest updateRequestPayload, TimeSpan timeout)
        {
            var currentSequence = (uint)Interlocked.Increment(ref _orchestratorSequenceCounter);
            ...
            var requestMain = new Device.Main {
             ...
             // The extractPayloadFunc needs to be smart enough to check the operation type within the UpdateFirmwareResponse
            return SendCommandAndAwaitSpecificResponseAsync(deviceId, serverId, requestMain, timeout,
                ...
                $"FirmwareUpdateOp-{updateRequestPayload.Operation}");
        }
